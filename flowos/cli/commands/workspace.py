"""
cli/commands/workspace.py -- FlowOS Workspace Commands

Provides the ``flowos workspace`` command group for managing Git-backed agent
workspaces.  This module re-exports the workspace command group that is
defined in ``cli/main.py`` so it can be registered on the root CLI from
either location.

The workspace commands are the primary interface for agents to manage their
local state boundary:

    flowos workspace init       -- Initialise a new workspace
    flowos workspace status     -- Show workspace status
    flowos workspace checkpoint -- Create a checkpoint commit
    flowos workspace revert     -- Revert to a checkpoint
    flowos workspace log        -- Show checkpoint history
    flowos workspace branch     -- Create a task branch
    flowos workspace handoff    -- Prepare a handoff
    flowos workspace archive    -- Archive the workspace
    flowos workspace diff       -- Show diff between refs
    flowos workspace git-log    -- Show Git commit log

All commands are implemented in ``cli/main.py`` on the ``workspace_group``
Click group.  This module imports and re-exports that group so it can be
registered from the new ``cli/commands/`` package structure.
"""
...
def workspace_group(ctx: click.Context) -> None:
    """Manage FlowOS agent workspaces (Git-backed state boundaries)."""
...
def workspace_init(
    ...
    for subdir in ("repo", ".flowos", "artifacts", "logs", "patches"):
        ...
        exists_marker = "[green]OK[/green]" if subdir_path.exists() else "[red]MISSING[/red]"
...
def workspace_status(ctx: click.Context) -> None:
    ...
    ws_status = workspace.get("status", "unknown")
    status_color = {
        "active": "green",
        "archived": "dim",
        "error": "red",
    ...
    console.print(
        Panel.fit(
            f"[dim]Workspace ID:[/dim] {workspace.get('workspace_id', '?')}\n"
            f"[dim]Agent ID:[/dim]     {workspace.get('agent_id', '?')}\n"
            f"[dim]Task ID:[/dim]      {workspace.get('task_id') or '(none)'}\n"
            f"[dim]Status:[/dim]       [{status_color}]{ws_status}[/{status_color}]\n"
            f"[dim]Type:[/dim]         {workspace.get('workspace_type', '?')}\n"
            f"[dim]Checkpoints:[/dim]  {len(checkpoints)}",
            title="[bold]Workspace Status[/bold]",
            border_style=status_color,
        )
    )

    console.print()
    dirty_marker = "[yellow]dirty[/yellow]" if git.get("is_dirty") else "[green]clean[/green]"
    console.print(
        Panel.fit(
            f"[dim]Branch:[/dim]   {git.get('branch', '?')}\n"
